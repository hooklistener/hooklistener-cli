...
- A skipped detector is a failed critique run unless `detect.mjs` is missing, crashes after a real attempt, or the target is terminal/TUI-only and the detector is not applicable.
- Viewable browser targets require browser inspection when available. Terminal/TUI targets require terminal evidence instead: snapshot tests, captured output, renderer tests, or screenshots across representative terminal sizes.
...
1. **Resolve the target** to a concrete file path or URL. Prefer a source path over a dev-server URL when both identify the same surface; ports drift, paths do not.
   ...
   - "the tunnel TUI" -> the renderer module plus snapshot tests
   - "the endpoint list command" -> the command handler plus output helper
2. **Compute the slug**:
   ```bash
...
Read relevant source files and visually inspect the live page when browser automation is available. For terminal/TUI targets, inspect captured command output, renderer snapshots, terminal screenshots, or run the command in representative terminal sizes when practical. Think like a design director.
...
For terminal/TUI-only targets, replace detector/browser evidence with terminal evidence:
- Run relevant snapshot tests or render tests.
- Capture representative command output or terminal screenshots when safe.
- Check narrow, normal, and wide terminal widths when the renderer is responsive.
- Scan for emoji, decorative Unicode, inconsistent status tokens, overflowing labels, broken wrapping, color-only state, and keyboard shortcut conflicts.
- Report `deterministic detector: not applicable (terminal/TUI target)` instead of treating skipped browser injection as a failure.
