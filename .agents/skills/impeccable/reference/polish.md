...
Detector and automated QA output are defect evidence only. A clean script result is never proof that the design is strong; gather browser evidence and inspect the real interaction path. For terminal/TUI targets, gather terminal evidence instead: rendered snapshots, command output, keyboard-path checks, and multiple terminal sizes.
...
1. **Find the design system**: Search for design system documentation, component libraries, style guides, or token definitions. For terminal/TUI work, include renderer modules, theme/color roles, output helper functions, table builders, snapshot tests, and sample command output. Study the core patterns: design principles, target audience, color tokens, spacing scale, typography styles, component API, motion conventions.
...
- **Terminal/TUI**: Assume monospace. Tune hierarchy through casing, weight/color roles, spacing, rules, alignment, and density instead of font changes.
...
- **Terminal/TUI**: Test at narrow, normal, and wide terminal widths; verify short-height behavior, scroll affordances, truncation, wrapping, and action menus.
