...
description: >
  ...
  otherwise improve a user interface. Covers frontend and terminal interfaces:
  websites, landing pages, dashboards, product UI, app shells, components,
  forms, settings, onboarding, empty states, CLI command output, terminal
  dashboards, TUIs, logs, prompts, status lines, tables, panels, and
  keyboard-driven workflows. Handles UX review, hierarchy, IA, cognitive load,
  accessibility, performance, responsive behavior across browser viewports or
  terminal sizes, theming, anti-patterns, typography, monospace ergonomics,
  spacing, layout, alignment, color, motion, UX copy, error states, edge cases,
  i18n, and design systems or tokens. Not for backend-only or non-UI tasks.
---

Designs and iterates production-grade interfaces, including browser frontends and terminal/TUI surfaces. Real working code, committed design choices, exceptional craft.
...
3. Familiarize yourself with any existing design system, conventions, and components in the code. Read at least one project file (CSS / tokens / theme / a representative component or page; for terminal/TUI work read the renderer, command-output helpers, theme/color module, snapshot tests, or sample terminal output). **Required even when you've loaded a sub-command reference in step 2.** Don't reinvent the wheel; use what's there when it works, branch out when the UX wins.
4. Read the matching register reference. **This is non-optional; skipping it produces generic output.** If the project is marketing, a landing page, a campaign, long-form content, or a portfolio (design IS the product), read `reference/brand.md`. If it is app UI, admin, a dashboard, a CLI, a TUI, or a tool (design SERVES the product), read `reference/product.md`. For terminal/TUI/CLI surfaces, also read `reference/tui.md`. Pick by first match: (1) task cue ("landing page" vs "dashboard" vs "terminal" / "CLI" / "TUI"); (2) surface in focus (the page, file, command, renderer, or route being worked on); (3) `register` field in PRODUCT.md.
5. **If the project is brand-new (no existing CSS tokens / theme / committed brand colors found in step 3)**, run `node .agents/skills/impeccable/scripts/palette.mjs` to receive a brand seed color and composition guidance. This is the anchor for your primary brand color. Compose the rest of the palette (bg, surface, ink, accent, muted) around it per the script's instructions. Use OKLCH throughout for browser UI. For terminal/TUI surfaces, translate the palette into ANSI-safe semantic roles and preserve legibility in 8-color, 16-color, and truecolor terminals where practical. **Skip this step only if step 3 found committed brand colors in existing tokens; in that case identity-preservation wins.**

### Terminal/TUI rule

When the target is a CLI, terminal dashboard, TUI, log view, command output, prompt, or keyboard workflow, browser-only requirements become terminal evidence requirements. Use snapshots, captured command output, terminal screenshots, golden tests, renderer tests, and multiple terminal sizes instead of browser screenshots and DOM inspection. Do not force web assumptions onto terminal software.
